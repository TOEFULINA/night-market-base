...
- `public/models/TRY5_SCENE.glb` is ~145MB (a few textures are intentionally kept HD - see below); this repo doesn't use Git LFS, so a fresh clone is a fairly large download. Worth moving to LFS (or a CDN) if that becomes a problem.
